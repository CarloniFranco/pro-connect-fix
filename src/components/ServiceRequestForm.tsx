...
export default function ServiceRequestForm({
  ...
  const handleSubmit = async () => {
    ...
    // Expira en 10 min si el cliente no completa el pago para liberar el turno rápido.
    const slotExpiresAt = new Date(Date.now() + 10 * 60 * 1000).toISOString();
          ...
          {totalPrice > 0 && (
            <div className="rounded-xl border-2 border-primary/30 bg-primary/5 p-3 space-y-1">
              ...
              <p className="text-[10px] text-muted-foreground pt-1 border-t border-primary/20">
                El resto (${Math.max(0, totalPrice - depositAmount).toLocaleString("es-AR")}) se paga al finalizar el servicio.
              </p>
            </div>
          )}
