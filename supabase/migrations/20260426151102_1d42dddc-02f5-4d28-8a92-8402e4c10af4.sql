-- Agregar columnas para guardar motivo de cancelación
ALTER TABLE public.service_requests
  ADD COLUMN IF NOT EXISTS cancellation_reason TEXT,
  ADD COLUMN IF NOT EXISTS cancelled_by TEXT;

-- Actualizar trigger para notificar al cliente cuando el profesional cancela un turno confirmado
...
BEGIN
  ...
  -- Rechazo del profesional ANTES de aceptar (declinar solicitud nueva/cotizada)
  IF NEW.status = 'rechazada_profesional' AND OLD.status IS DISTINCT FROM 'rechazada_profesional' AND NEW.client_user_id IS NOT NULL THEN
    -- Si era un turno ya confirmado (aceptada/en_servicio), tratamos como cancelación de turno
