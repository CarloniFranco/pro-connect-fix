...
DECLARE
  ...
  cancel_title TEXT;
  cancel_message TEXT;
  pro_cancel_title TEXT;
  pro_cancel_message TEXT;
...
BEGIN
  ...
  IF NEW.status = 'rechazada_profesional' AND OLD.status IS DISTINCT FROM 'rechazada_profesional' AND NEW.client_user_id IS NOT NULL THEN
    IF OLD.status IN ('aceptada', 'en_servicio') THEN
      pro_cancel_title := 'El profesional canceló tu turno';
      pro_cancel_message := COALESCE(pro_name_val, 'El profesional')
        || ' tuvo que cancelar tu turno de ' || COALESCE(NEW.service_type, 'servicio')
        || '. Lamentamos los inconvenientes — la seña te será reintegrada en los próximos días.';
      IF NEW.cancellation_reason IS NOT NULL AND length(trim(NEW.cancellation_reason)) > 0 THEN
        pro_cancel_message := pro_cancel_message || ' Motivo: ' || NEW.cancellation_reason;
      END IF;
      INSERT INTO notifications (user_id, type, title, message, link, service_request_id)
      VALUES (
        NEW.client_user_id, 'profesional_cancelo', pro_cancel_title, pro_cancel_message,
        '/mis-pedidos', NEW.id
      );
    ELSE
      INSERT INTO notifications (user_id, type, title, message, link, service_request_id)
  ...
  IF NEW.status = 'aceptada' AND OLD.status IS DISTINCT FROM 'aceptada' THEN
    ...
    -- Notificar al cliente que su pedido quedó confirmado (sin seña, ej. pros sin MP)
    IF NEW.client_user_id IS NOT NULL AND COALESCE(NEW.deposit_paid, false) = false THEN
      confirm_message := 'Tu pedido de ' || COALESCE(NEW.service_type, 'servicio') || ' con '
        || COALESCE(pro_name_val, 'el profesional') || ' quedó confirmado.';
      INSERT INTO notifications (user_id, type, title, message, link, service_request_id)
      VALUES (
        NEW.client_user_id, 'pedido_confirmado', '¡Pedido confirmado!', confirm_message,
        '/mis-pedidos', NEW.id
      );
    END IF;
  END IF;
  ...
  IF NEW.deposit_paid = true AND (OLD.deposit_paid IS DISTINCT FROM true) THEN
    ...
    -- Notificar al cliente que su pedido quedó confirmado tras pagar la seña
    IF NEW.client_user_id IS NOT NULL THEN
      confirm_message := 'Tu seña fue recibida y tu turno de ' || COALESCE(NEW.service_type, 'servicio')
        || ' con ' || COALESCE(pro_name_val, 'el profesional') || ' quedó confirmado.';
      INSERT INTO notifications (user_id, type, title, message, link, service_request_id)
      VALUES (
        NEW.client_user_id, 'pedido_confirmado', '¡Pedido confirmado!', confirm_message,
        '/mis-pedidos', NEW.id
